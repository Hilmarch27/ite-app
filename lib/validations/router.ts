...
export const RouterFormSchema = z.object({
  ...
  routerSeries: z.string().min(1, { message: "Please select a router series" }).trim(),
  ...
  information: z.string().min(1, { message: "Please select a information" }).trim(),
});

export type RouterFormType = z.infer<typeof RouterFormSchema>;